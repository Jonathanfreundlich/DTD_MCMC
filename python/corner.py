...
def corner(xs, bins=20, range=None, weights=None, color="k",
           ...
           truths=None, truth_color="#4682b4",percentiles=None,percentiles_color='r',
           scale_hist=False, quantiles=None, verbose=False, fig=None,
           max_n_ticks=5, top_ticks=False, use_math_text=False,fontsize=16,
    ...
    **hist2d_kwargs
    ...
    for i, x in enumerate(xs):
        ...
        # Plot the histograms.
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights,zorder=5,
                              range=np.sort(range[i]), **hist_kwargs)
        else:
            if gaussian_filter is None:
                raise ImportError("Please install scipy for smoothing")
            if smooth_histogram==True:
                n, b = np.histogram(x, bins=bins[i], weights=weights,range=np.sort(range[i]))
                n = gaussian_filter(n, smooth1d)
                x0 = np.array(list(zip(b[:-1], b[1:]))).flatten()
                y0 = np.array(list(zip(n, n))).flatten()
                ax.plot(x0[::2], y0[::2],zorder=5,**hist_kwargs)
            else:
                n, b = np.histogram(x, bins=bins[i], weights=weights,range=np.sort(range[i]))
                n = gaussian_filter(n, smooth1d)
                x0 = np.array(list(zip(b[:-1], b[1:]))).flatten()
                y0 = np.array(list(zip(n, n))).flatten()
                ax.plot(x0, y0,zorder=5, **hist_kwargs)                

        if truths is not None and truths[i] is not None:
            ax.axvline(truths[i], color=truth_color,zorder=10)

        if percentiles is not None and percentiles[i] is not None:
            ax.axvline(percentiles[i][1], color=percentiles_color,ls='--',dashes=(7,5),zorder=-1)
            ax.axvline(percentiles[i][0], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
            ax.axvline(percentiles[i][2], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
        ...
        for j, y in enumerate(xs):
            ...
            if truths is not None:
                if truths[i] is not None and truths[j] is not None:
                    ax.plot(truths[j], truths[i], "s", color=truth_color,zorder=10)
            ...
            if percentiles is not None:
                if percentiles[j] is not None:
                    ax.axvline(percentiles[j][1], color=percentiles_color,ls='--',dashes=(7,5),zorder=-1)
                    ax.axvline(percentiles[j][0], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
                    ax.axvline(percentiles[j][2], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
                if percentiles[i] is not None:
                    ax.axhline(percentiles[i][1], color=percentiles_color,ls='--',dashes=(7,5),zorder=-1)
                    ax.axhline(percentiles[i][0], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
                    ax.axhline(percentiles[i][2], color=percentiles_color,ls='--',dashes=(2,3),zorder=-1)
            ...
            if i < K - 1:
                ...
                [l.set_rotation(45) for l in ax.get_xticklabels()]
                if labels is not None:
                    ax.set_xlabel(labels[j],fontsize=fontsize, **label_kwargs)
                    ax.xaxis.set_label_coords(0.5, -0.2)
            ...
            if j > 0:
                ...
                if labels is not None:
                    ax.set_ylabel(labels[i],fontsize=fontsize, **label_kwargs)
